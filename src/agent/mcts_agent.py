...
#         current_player = root.board.turn
...
#                     expansion_nodes.append((move, new_state, board))
#                     tensor = self.board_to_tensor(board)
#                     expansion_tensors.append(tensor)

#                 node.untried_moves = []

#                 # Batch inference
#                 batch_tensor = torch.stack(expansion_tensors)
#                 with torch.no_grad():
#                     policy_logits, values = self.model(batch_tensor)
#                     policy_probs = torch.softmax(policy_logits, dim=1).cpu().numpy()
#                     values = values.squeeze().cpu().numpy()

#                 for i, (move, new_state, board) in enumerate(expansion_nodes):
#                     move_idx = self.move_to_index(move)
#                     prior = policy_probs[i][move_idx]
#                     value = values[i]
#                     child_node = MCTSNode(
...
#                 # Pick one child to continue simulation
#                 move, new_state, board = expansion_nodes[0]
